...
class ResumoRepository:
    ...
    @staticmethod
    def upsert(mes_referencia, total_entradas, total_saidas, total_quebrados, faturamento_total, lucro_estimado):
        """Insere ou atualiza o resumo de um mês."""
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute(
            """INSERT INTO resumo_mensal
                   (mes_referencia, total_entradas, total_saidas, total_quebrados, faturamento_total, lucro_estimado)
               VALUES (?, ?, ?, ?, ?, ?)
